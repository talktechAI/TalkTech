...
import { getEnvWithDefaults } from '@/utils/cloudflare-context';
...
export async function POST(request: NextRequest) {
  try {
    // Safely get environment - works both on Cloudflare and locally
    ...
    // Store in D1 database if configured (only available on Cloudflare)
    if (env.TALKTECH_DB) {
      ...
      // In development, just log the contact
      console.log('Contact submission (D1 not available):', body);
    }
